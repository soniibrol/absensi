...
class Attendance extends CI_Controller{
	public function __construct(){
		parent::__construct();
		$this->load->model(array('master/alasan_m','attendance_m','presences_m'));
	...
	public function request(){
		...
		$this->form_validation->set_rules('nik_atasan','NIK Atasan','required');
		...
		if($this->form_validation->run()==false){
			...
			if(intval($this->input->post('count_alasan_request')) > 0){
				...
				$this->session->set_flashdata('message_alert','<div class="alert alert-danger">You have to fill at least 1 reason.</div>');
				redirect('attendance/request');
			}
		}
	}
	...
	public function aborting($id_absen_susulan){
		if(!$id_absen_susulan){
			...
			if($count > 0){
				...
				if($tmp['id_karyawan']==$this->session->userdata('user_id')){
					$data_absen_susulan = array(
									'active' => '0',
		...
		if(!$id_absen_susulan){
			...
			if($count > 0){
				...
				if($tmp['id_atasan']==$this->session->userdata('user_id')){
					...
					$cek = $this->presences_m->get_by_date($tmp['tanggal_absen'],$tmp['id_karyawan'])->num_rows();
					if($cek > 0){
						//jika sudah ada, beri pesan ke user bahwa jam masuk telah di input
						$dt_kehadiran = $this->presences_m->get_by_date($tmp['tanggal_absen'],$tmp['id_karyawan'])->row_array();
						$data_kehadiran = array(
											'id_karyawan' => $tmp['id_karyawan'],
